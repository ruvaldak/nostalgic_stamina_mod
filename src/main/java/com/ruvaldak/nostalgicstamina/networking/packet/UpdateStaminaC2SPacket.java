    ...
    public static void receive(MinecraftServer server, ServerPlayerEntity player, ServerPlayNetworkHandler handler, PacketByteBuf buf, PacketSender responseSender) {
        ...
        if (player.isSprinting() || player.isSwimming()) {
            if (!player.isCreative() && !player.isSpectator()) {
                ...
                if(System.currentTimeMillis() - NostalgicStaminaMod.getClientStoredSystemMillis()+rechargeDelayInMillis >= 0) {
                    if(ConfigRegistry.CONFIG.extraLogging) 
                        //extra Logging
                        NostalgicStaminaMod.LOGGER.info("Setting recharge countdown to: " + Math.floor(ConfigRegistry.CONFIG.rechargeDelayInSeconds * 100) / 100d + " seconds.");
                    
                    // if expending stamina, set countdown to configurable recharge delay
                    NostalgicStaminaMod.updateClientStoredSystemMillis();
                }
                serverState.markDirty();
            }
        } 
        else if (!player.isSprinting() && !player.isSwimming() && !player.isClimbing() && playerstate.getStamina() < playerstate.getMaxStamina()) {
            if(System.currentTimeMillis() >= (NostalgicStaminaMod.getClientStoredSystemMillis()+rechargeDelayInMillis)) {
